        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if (!empty($row['main_product'])) {
                ...
                foreach ($main_product_ids as $product_id) {
                    ...
                    if ($product_id > 0) {
                        $product_query = "SELECT id, tieu_de, minh_hoa, gia_cu, gia_moi, link, shop FROM sanpham WHERE id = $product_id AND active = 0 AND kho >= 0 AND ((NOT EXISTS (SELECT 1 FROM phanloai_sanpham pl WHERE pl.sp_id = sanpham.id) AND kho > 0) OR EXISTS (SELECT 1 FROM phanloai_sanpham pl WHERE pl.sp_id = sanpham.id AND pl.kho_sanpham_socdo > 0)) LIMIT 1";
