...
class _VoucherSectionState extends State<VoucherSection> {
  ...
  Future<void> _showPlatformVoucherDialog(BuildContext context) async {
    ...
    final vouchers = await api.getVouchers(
      type: 'platform',
    );
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Chọn Voucher sàn', style: TextStyle(fontWeight: FontWeight.w700)),
              const SizedBox(height: 12),
              Flexible(
                child: ListView.separated(
                  ...
                  itemBuilder: (context, i) {
                    final v = vouchers[i];
                    final eligibility = _eligibilityForPlatformVoucher(v, items);
                    ...
                    return ListTile(
                      ...
                      leading: Icon(Icons.local_activity, color: canUse ? Colors.red : Colors.grey),
                      title: Text('${v.code} · ${_discountText(v)}',
                          style: TextStyle(color: canUse ? Colors.black87 : Colors.grey)),
                      subtitle: Text(minTxt, style: TextStyle(color: canUse ? Colors.grey[700] : Colors.grey)),
                      trailing: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          if (!canUse)
                            IconButton(
                              ...
                              onPressed: () => _showIneligibleReason(context, v, eligibility.$2),
                            ),
                          Container(
                            ...
                            child: Text(canUse ? 'Sử dụng' : 'Chưa đủ điều kiện',
                                style: const TextStyle(color: Colors.white, fontSize: 11)),
                          ),
                        ],
                      ),
                      onTap: canUse ? () => Navigator.pop(context, v) : () => _showIneligibleReason(context, v, eligibility.$2),
                    );
                  },
    ...
    if (selected != null) {
      ...
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Đã áp dụng voucher sàn ${selected.code}')),
      );
    }
  }
  ...
  (bool, String) _eligibilityForPlatformVoucher(Voucher v, List<cart_service.CartItem> items) {
    // 1) HSD / trạng thái
    ...
    if (v.minOrderValue != null && subtotal < v.minOrderValue!.round()) {
      return (false, 'Đơn tối thiểu ${FormatUtils.formatCurrency(v.minOrderValue!.round())}.');
    }

    // 3) Applicable products (nếu có)
    final allowIds = <int>{};
    if (v.applicableProductsDetail != null && v.applicableProductsDetail!.isNotEmpty) {
      for (final m in v.applicableProductsDetail!) {
        final id = int.tryParse(m['id'] ?? '');
        if (id != null) allowIds.add(id);
      }
    } else if (v.applicableProducts != null && v.applicableProducts!.isNotEmpty) {
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  Text('${v.code} · ${_discountText(v)}', style: const TextStyle(fontWeight: FontWeight.w700)),
                  const SizedBox(height: 8),
                  if (v.minOrderValue != null)
                    Text('• Đơn tối thiểu: ${FormatUtils.formatCurrency(v.minOrderValue!.round())}'),
                  if ((v.applicableProductsDetail?.isNotEmpty ?? false) || (v.applicableProducts?.isNotEmpty ?? false)) ...[
                    const SizedBox(height: 8),
                                  ...
                                  ClipRRect(
                                    ...
                                    child: Container(
                                      ...
                                      child: item.$3.isNotEmpty
                                          ? Image.network(item.$3, fit: BoxFit.contain,
                                              errorBuilder: (_, __, ___) => const Icon(Icons.image_not_supported, color: Colors.grey))
                                          : const Icon(Icons.image_not_supported, color: Colors.grey),
                                    ),
                                  ),
                                  const SizedBox(height: 6),
                                  Text(item.$2, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12)),
                                ],
                              ),
                            ),
                          );
                  ...
                  if (reason.isNotEmpty) ...[
                    const SizedBox(height: 12),
                    Text('Lý do hiện tại: $reason', style: const TextStyle(color: Colors.red)),
                  ],
                  const SizedBox(height: 8),
                ],
              ),
